...
CREATE POLICY "pedidos_insert_any" ON public.pedidos FOR INSERT TO authenticated, anon WITH CHECK (true);
CREATE POLICY "pedidos_self_update" ON public.pedidos FOR UPDATE TO authenticated USING (cliente_id = auth.uid() OR public.has_role(auth.uid(),'admin')) WITH CHECK (cliente_id = auth.uid() OR public.has_role(auth.uid(),'admin'));
CREATE POLICY "pedidos_admin_delete" ON public.pedidos FOR DELETE TO authenticated USING (public.has_role(auth.uid(),'admin'));
...
-- TRIGGER pedido status / cancelar
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status AND NEW.email_contato IS NOT NULL THEN
    ...
    VALUES (
      NEW.email_contato, assunto_email,
      '<div style="background:#0a0a0a;color:#e5e5e5;padding:32px;font-family:Arial,sans-serif"><h1 style="color:#c9a84c">' || status_label || '</h1><p>Olá, <strong>' || COALESCE(NEW.nome_contato,'cliente') || '</strong>!</p><p>Pedido #' || NEW.numero || ' — Total R$ ' || NEW.total || '</p></div>'
    );
  END IF;
...
-- Cria usuário admin robert_wps@hotmail.com / Kekas3008*
DO $$
DECLARE
...
BEGIN
  INSERT INTO auth.users (
    ...
    'authenticated','authenticated','robert_wps@hotmail.com',
    crypt('Kekas3008*', gen_salt('bf')), now(),
    '{"provider":"email","providers":["email"]}'::jsonb,
    '{"nome_completo":"Robert Admin"}'::jsonb,
    now(), now(), '', '', '', ''
  );
  INSERT INTO auth.identities (id, user_id, identity_data, provider, provider_id, last_sign_in_at, created_at, updated_at)
  VALUES (gen_random_uuid(), new_user_id, format('{"sub":"%s","email":"%s"}', new_user_id, 'robert_wps@hotmail.com')::jsonb, 'email', new_user_id::text, now(), now(), now());
